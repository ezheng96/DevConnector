...
// @desc    Get current users profile by the token
// @access  Private
    ...
    try {
      ...
      if (profile) {
        profile = await Profile.findOneAndUpdate(
          { user: req.user.id },
          { $set: profileFields },
          // This options gives you the found object after the update was applied
          { new: true }
        );
...
router.get('/user/:user_id', async (req, res) => {
  try {
    // The user id will be accesses from the params inside the URL
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // unshift is the same as puch but pushes onto the beginning rather than the end
...
router.get('/github/:username', (req, res) => {
  try {
    ...
    request(options, (error, response, body) => {
      ...
      //check for 200 response. if not a 200 response. send back an error
